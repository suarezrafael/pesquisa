...
- **Testado ao vivo antes de considerar "pronto"** — só configurar o secret e assumir que funciona
  ...
  de teste do usuário. Só depois de confirmar isso disparei o cron de
...
- **Testado ao vivo com um envio real**: `curl` direto na API do Resend com
  ...
  família(s) elegível(is)` — e-mail real enviado pra a única família elegível, que é a conta de
  teste do próprio usuário.
