# Laboratório 18 — Polimento: física do pulo, animação de andar, cor dos morros
...
- [ ] Ajustar `GRAVITY`/`JUMP_SPEED` pra um pulo mais "no chão", preservando folga confortável
      sobre os degraus do parkour (lab-11, altura 0.85) e alcance horizontal suficiente entre
      plataformas (2.27).
- [ ] Trocar a fórmula de dobra de joelho (jogador e NPCs) por uma que nunca fica 100% reta
      durante a caminhada — dobra contínua entre um mínimo e um máximo, não clipada em zero
      durante metade do ciclo.
- [ ] Cor de morro por altura (não só por inclinação) no vértice do planeta — platôs ganham
...
- [ ] Verificação: `npm run build` passa; testar os três ajustes rodando o dev server (pulo com
      tempo no ar/altura reduzidos mas parkour ainda alcançável, joelho dobrando visivelmente
      durante toda a caminhada, platô com cor de morro visível numa captura de tela).
