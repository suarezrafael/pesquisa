...
- [ ] Pato no rio: reaproveita `buildPato()`, nada pra frente/trás ao longo de `riverCenter`
      (mesmo padrão de `pathIndex` já usado pelos carros), sem trocar nada existente da lagoa.
- [ ] Rua vira um laço fechado: em vez do arco curto atual (theta 280°-320°, phi variando junto),
      ...
      `closePath: true`.
- [ ] Carros de IA (`carros`, lab-15) passam a dar voltas contínuas no laço fechado (sem mais
      ricochetear nas pontas — um laço fechado não tem "ponta").
- [ ] Entrar/sair do carro: tecla nova (`e`) perto de um carro (distância curta) alterna estado
...
- [ ] Dirigir: setas/WASD cima/baixo avançam/recuam o carro ao longo da rua (`pathIndex`), mesmo
...
- [ ] Verificação: `npm run build` passa; testar ao vivo — andar até um carro, apertar `e`, andar
      pra frente/trás com as setas, dar uma volta completa na rua (sair pelo fim do array e
      continuar do início sem travar), apertar `e` de novo pra sair.
